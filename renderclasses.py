...
class enemy:
    __slots__ = ("pos", "speed", "type", "facing", "texture", "texsize", "center", "maxsize")
    ...
    def draw(self, screen, plr, fovdistance, w, h, lightsources: list[light], ambient: light, near: float):
        rpos = (self.pos - plr.pos)
        rpos = rpos@plr.rmat
        self.center = rpos.z
        if infrustum(rpos, fovdistance, w, h, near):
            imagescale = 5/mt.sqrt(glm.dot((self.pos-plr.pos), (self.pos-plr.pos)))
            if self.texsize.x*imagescale<4 or self.texsize.y*imagescale<4 or imagescale>3:
                return 0
    ...
    def update(self, dt, plr):
        self.facing = glm.normalize(plr.pos-self.pos-2*glm.normalize(plr.pos-self.pos))
...
class triangle:
    __slots__ = ("a", "b", "c", "n", "rota", "rotb", "rotc", "center", "clr", "refl", "objectfrustumcheck")

    def __init__(self, a:glm.vec3, b:glm.vec3, c:glm.vec3, n:glm.vec3, clr:glm.vec3, reflectance:float):
        self.a = a
        self.b = b
        self.c = c
    ...
    def draw(self, screen:pg.surface.Surface, plr:player, distance: float, w: int, h: int, lightsources, ambient: light, near: float):
